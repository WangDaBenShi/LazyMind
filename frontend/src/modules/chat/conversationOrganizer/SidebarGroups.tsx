...
type Props = { batchSelection?: GroupBatchSelection; namesLocked?: boolean; groups: ConversationGroup[]; searchText?: string; currentConversationId?: string; onNew?: (id: string) => void; onEdit: (group: ConversationGroup | "new") => void; onRemove: (group: ConversationGroup) => void };
export default function SidebarGroups({ groups, searchText = "", currentConversationId, onNew, onEdit, onRemove, namesLocked = false, batchSelection }: Props) {
  ...
  const block = (g: ConversationGroup) => {
    ...
    return <ConversationGroupDropZone key={g.id} groupId={g.id} groupName={g.name} disabled={Boolean(batchSelection || searchText || busy)} className={`conversation-group ${dropTarget === g.id ? "group-drop-target" : ""}`} onDragOver={e => { if (!batchSelection && !searchText && !busy && (e.dataTransfer.types.includes(CONVERSATION_DRAG) || e.dataTransfer.types.includes(GROUP_DRAG))) { e.preventDefault(); setDropTarget(g.id); } }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDropTarget(""); }} onDrop={async e => {
      ...
      try {
        ...
        else if (conversation && conversation.groupId !== g.id) { await moveMember(g.id, conversation.id); }
      } catch { /* The shared request interceptor displays the API error. */ }
    }}>
      <div className={`conversation-group-row ${selected ? "is-selected" : ""}`} draggable={!batchSelection && !searchText && !busy} onDragStart={e => { e.dataTransfer.setData(GROUP_DRAG, g.id); e.dataTransfer.effectAllowed = "move"; }} onDragEnd={() => { setDropTarget(''); setMemberDropTarget(null); }}>
        ...
        <button className="conversation-group-name" title={g.name} aria-expanded={open} onClick={() => toggle(setCollapsed, g.id)}>{open ? <FolderOpenOutlined /> : <FolderOutlined />}<span>{g.name}</span></button>
      ...
      {open && <div className="conversation-group-members">
        {visible.map(c => <ConversationPreview key={c.conversation_id} conversationId={c.conversation_id} title={c.display_name || c.conversation_id} summary={c.summary} updateTime={c.updated_at} isTask={Boolean(c.is_task_conv)} disabled={Boolean(batchSelection) || renamingId === c.conversation_id}><div className={`conversation-group-member ${c.conversation_id === currentConversationId ? "active" : ""} ${memberDropTarget?.id === c.conversation_id ? `member-drop-${memberDropTarget.position}` : ''}`} draggable={renamingId !== c.conversation_id && !batchSelection && !searchText && !busy}
          ...
          onDragOver={e => {
            if (batchSelection || searchText || busy || !e.dataTransfer.types.includes(CONVERSATION_DRAG)) return;
          ...
          onDrop={e => {
            ...
            if (batchSelection || searchText || busy) return;
  ...
  return <>
    ...
    <div className="conversation-groups-heading"><span>{t("conversationOrganizer.groups")}</span>{!batchSelection && <Button type="text" size="small" icon={<PlusOutlined />} disabled={namesLocked} title={namesLocked ? t("conversationOrganizer.namesLocked") : undefined} aria-label={t("conversationOrganizer.newGroup")} onClick={() => onEdit("new")} />}</div>
