...
describe("knowledge market background tasks", () => {
  it.each([
    ["install", "安装", "安装中"],
    ["update", "检查更新", "更新中"],
  ])("labels a submitted %s by task type while parsing is active", async (id, action, label) => {
    await mountPage();
    ...
    await click(card);
    expect(within(screen.getByRole("dialog")).getByRole("button", { name: new RegExp(`${label}$`) })).toBeDisabled();
  });

  it("restores the updating label after the update job succeeds but parsing is still active", async () => {
  ...
  it("uses a neutral processing label when the active job type is unavailable", async () => {
    mockActiveInstall("update");
    await mountPage();
    await click(screen.getByRole("tab", { name: /知识广场/ }));
    const card = screen.getByRole("button", { name: "查看知识库 update详情" });
