...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	if len(v03.Dev) != 74 {
		t.Fatalf("v0_3 dev migration count=%d, want 74", len(v03.Dev))
	}
	if !containsMigrationFileVersion(v03.Dev, 20260914090000) {
		t.Fatal("v0_3 dev migrations are missing document learning capabilities")
	}
